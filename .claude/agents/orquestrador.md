...
- **QA** (`qa`, agente do projeto) — validação e teste do sistema: roda a pirâmide completa (pytest, vitest, E2E Playwright em `e2e/`), verifica regras de negócio da spec e produz relatórios PDF com status e evidências. Reporta bugs sem corrigi-los. Prefira este ao `fabrica-agentes:playwright-tester` para validações do projeto.
